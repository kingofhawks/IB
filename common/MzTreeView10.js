...
function MzTreeView(Tname)
{
  ...
  this.toString = function()
  {
    ...
    if(rootCN.length>0)
    {
      ...
      setTimeout(this.name +".focusClientNode('"+ rootCN[0].id +"'); ",10);
    }
...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  var HTML = "<DIV noWrap=\"True\" onmouseover=\"this.style.backgroundColor='#FFCC99'\" onmouseout=\"this.style.backgroundColor=''\"><NOBR>";
  if(!isRoot)
  {
...
MzTreeView.prototype.nodeClick = function(id)
{
  ...
  this.getAttribute(source, "url") ? rHelp(this.getAttribute(source, "url")) : "";
