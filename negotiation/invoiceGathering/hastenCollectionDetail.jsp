...
function switchchco(flag)
{
	switch ($("chco").value)
	{
		...
		default:
            ...
            getCharge("caam","01",$("dlam").value,$("prid").value);
	}
}
	...
	<tr>
		...
		<table width="100%" height="100%" border="0" align="center"
			...
			<tr>
				...
				<div id="innerTable_div" ><!--内容开始--> <html:form action="EBTR.do"
					...
					<table width="100%" border="0" cellpadding="0" cellspacing="0">
						<tr>
							...
							<table width="375" border="0" cellpadding="0" cellspacing="0">
								...
								<tr>
									<td height="20" align="right" valign="top">邮电费币别：</td>
									<td valign="top">
									<div class="sele_div"><html:select styleClass="coin"
										property="tlcy">
										<option></option>
      ...
      if(draftFlag){%>
		...
		onClick="openReplace('/IB/voucher.do?prid=<bean:write name="hastenCollectionDetailForm" property="prid"/>&step=<%=step%>&flag=flag&rfno=<bean:write name="hastenCollectionDetailForm" property="rfno"/>&no=<bean:write name="hastenCollectionDetailForm" property="bpno"/>');return false">&nbsp;&nbsp;
		
		<%if(tabID!=null&&tabID.equals("6")){//流程图，只能在以生效里面看到%>
		<input	type="image" src="<bean:message key="common.svg" />"
		onClick="window.open('/IB/flowchart.do?id=<bean:write name="hastenCollectionDetailForm" property="bpno"/>&prid=<bean:write name="hastenCollectionDetailForm" property="prid"/>','newwindow','toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no');return false">&nbsp;&nbsp;
		<%}
          }
		...
		onClick="openReplace('invoiceGatheringManageAction.do?tabID=<%=tabID%>&prid=<bean:write name="hastenCollectionDetailForm" property="prid"/>');return false">
