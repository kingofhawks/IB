...
<table width="100%" height="100%" border="0" align="center"
	...
	<tr>
		...
		<td class="outerTable_head"><script language="javascript">showTab("<%=request.getAttribute("opid")%>");</script>
			<%
	//	String tabID=(String)request.getAttribute("tabID");
	//	if(tabID!=null&&Integer.parseInt(tabID)<4)//在电文、经办、暂存、待修改目录
	//	{%>
<%--		<script language="javascript">showOperation("report2.do?flag=flag&prid=<%=prid%>&type=N");</script>--%>
		<%//}%>
		</td>
		...
		<table width="100%" height="100%" border="0" align="center"
			...
			<tr>
				...
				<div id="innerTable_divLis" style="width:96%"><!-内容开始--><%String cost = com.witsoft.common.Constant.isHaveReceiveDraft(prid,(String)request.getAttribute("tabID"));%><script>showDone("<%if(request.getAttribute("msg")!=null){out.print(request.getAttribute("msg"));}%>");</script> <logic:equal
					...
					<table width="100%" border="0" cellpadding="0" cellspacing="1"
						id="item_list">
						<tr>
						    <th width="26%"><bean:message key="common.lcno" /><!--信用证号--></th>
							<th width="26%"><bean:message key="common.adno" /><!--通知号--></th>
							<th width="22%"><bean:message key="common.clno" /><!--客户号--></th>
							<th width="26%">银行编号<!--机构号--></th>
						</tr>
						<logic:notEmpty name="list">
							<logic:iterate id="list" name="list" indexId="index" scope="request">

								<span
									onclick="openReplace('report2.do?flag=flag&rfno=<bean:write
                                        name="list" property="id.adno"/>&prid=<%=prid%>&brno=<bean:write
                                        name="list" property="id.brno"/>')">
            ...
            out.print("<tr bgcolor=\"#ECF4FC\" onmouseover=\"this.style.backgroundColor=\'#FFCC99\';\" onmouseout=\"this.style.backgroundColor=\'#ECF4FC\';\">");
             ...
             out.print("<tr bgcolor=\"#ffffff\" onmouseover=\"this.style.backgroundColor=\'#FFCC99\';\" onmouseout=\"this.style.backgroundColor=\'#ffffff\';\">");
									...
									<td><bean:write name="list" property="id.adno" /></td>
...
<logic:notEqual name="userCommonForm"
					...
					<table width="100%" border="0" cellpadding="0" cellspacing="1"
						id="item_list">
						<tr>
							<th width="17%"><bean:message key="common.bpno" /><!--议付编号--></th>
							<th width="18%"><bean:message key="common.rfno" /><!--流水号--></th>
							<th width="17%"><bean:message key="common.adno" /><!--通知号--></th>
							<th width="16%"><bean:message key="common.clno" /><!--客户号--></th>
							<th width="16%">银行编号<!--机构号--></th>
							<th width="16%"><bean:message key="common.dldt" /><!--处理日期--></th>
						</tr>
						<logic:notEmpty name="list">
							<logic:iterate id="list" name="list" indexId="index" scope="request">
								<span
									<%=cost%>="openReplace('<%=prid%>.do?rfno=<bean:write
                                        name="list" property="id.rfno" />&prid=<%=prid%>&type=<bean:write name="list" property="type" />')"><%if ((index.intValue() % 2) == 1) {
            out.print("<tr bgcolor=\"#ECF4FC\" onmouseover=\"this.style.backgroundColor=\'#FFCC99\';\" onmouseout=\"this.style.backgroundColor=\'#ECF4FC\';\">");
             } else {
             out.print("<tr bgcolor=\"#ffffff\" onmouseover=\"this.style.backgroundColor=\'#FFCC99\';\" onmouseout=\"this.style.backgroundColor=\'#ffffff\';\">");
              }%>
