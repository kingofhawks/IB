...
<table width="100%" height="100%" border="0" align="center" cellpadding="0" cellspacing="0">
    ...
    <tr>
                ...
                <tr>
                    ...
                    <td align="center" class="innerTable_main">
                        <div id="innerTable_div">
                            ...
                            <!--内容开始-->
                            <html:form action="OCAC.do" onsubmit="checkForm(this)" method="post">
                                ...
                                <tr>
									...
									<html:option value="<%=cyno[0]%>"><%=cyno[1]%></html:option>	
                                            ...
                                            <tr>
                                                ...
                                                <td valign="top"><html:text title="amts" property="amts" maxlength="15" onkeypress="regInput(/^\d{1,}\.?\d{0,2}$/)" style="ime-mode:Disabled"/></td>
                                            </tr>
                                            <tr>
                                                <td height="20" align="right" valign="top" class="attR">*到期日：</td>
                                                <td valign="top">
													<html:text styleClass="date" property="dudt" readonly="true" title="dudt"/>
      ...
      if(draftFlag){%>
		...
		%>
		<input type="image" src='<bean:message key="common.report" />' onClick="openReplace('/IB/voucher.do?prid=<bean:write name="outwardCollectionAcceptanceCheckInDetailForm" property="prid"/>&step=<%=step%>&flag=flag&rfno=<bean:write name="outwardCollectionAcceptanceCheckInDetailForm" property="rfno"/>&no=<bean:write name="outwardCollectionAcceptanceCheckInDetailForm" property="ocno"/>');return false">
		&nbsp;&nbsp;
		
		<%if(tabID!=null&&tabID.equals("6")){//流程图，只能在以生效里面看到%>
		<input type="image" src='<bean:message key="common.svg" />' onClick="window.open('/IB/flowchart.do?id=<bean:write name="outwardCollectionAcceptanceCheckInDetailForm" property="ocno"/>&prid=<bean:write name="outwardCollectionAcceptanceCheckInDetailForm" property="prid"/>','newwindow','toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no');return false">
		&nbsp;&nbsp;
		<%}
          }
		...
		<%}%>
        <input type="image" src='<bean:message key="common.returnList" />' onClick="openReplace('outwardCollectionManageAction.do?tabID=<%=tabID%>&prid=<bean:write name="outwardCollectionAcceptanceCheckInDetailForm" property="prid"/>');return false">
