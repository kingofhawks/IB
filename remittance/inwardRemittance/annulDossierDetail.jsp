...
<table width="100%" height="100%" border="0" align="center" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    ...
                    <td align="center" valign="middle" class="innerTable_main">
                        <div id="innerTable_div">
                            <!--录入信息表格开始-->
                            <html:form action="IRCL.do"  onsubmit="checkForm(this)" method="post">
   ...
   <tr>
      ...
      if(draftFlag){%>
		...
		%>
		<input type="image" src='<bean:message key="common.report" />' onClick="openReplace('/IB/voucher.do?prid=<bean:write name='annulDossierDetailActionForm' property='prid'/>&step=<%=step%>&flag=flag&rfno=<bean:write name='annulDossierDetailActionForm' property='rfno'/>&no=<bean:write name='annulDossierDetailActionForm' property='irno'/>');return false">&nbsp;&nbsp;
						        <%if(tabID!=null&&tabID.equals("6")){//流程图，只能在以生效里面看到%>
						        <input	type="image" src='<bean:message key="common.svg" />' onClick="window.open('/IB/flowchart.do?id=<bean:write name='annulDossierDetailActionForm' property='irno'/>&prid=<bean:write name='annulDossierDetailActionForm' property='prid'/>','newwindow','toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no');return false">&nbsp;&nbsp;
						        <%}
          }
		if(tabID!=null&&Integer.parseInt(tabID)>3&&Integer.parseInt(tabID)<6){//在复核、授权显示返回经办按钮%>
						        ...
						        &nbsp;&nbsp;
						        <%}%>
						        <input	type="image" src='<bean:message key="common.returnList" />' onClick="openReplace('inwardManageAction.do?tabID=<%=tabID%>&prid=<bean:write name='annulDossierDetailActionForm' property='prid'/>');return false">
